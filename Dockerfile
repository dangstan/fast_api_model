...
RUN --mount=type=secret,id=AWS_ACCESS_KEY_ID,dst=/etc/secrets/AWS_ACCESS_KEY_ID cat /etc/secrets/AWS_ACCESS_KEY_ID && \
    --mount=type=secret,id=AWS_SECRET_ACCESS_KEY,dst=/etc/secrets/AWS_SECRET_ACCESS_KEY cat /etc/secrets/AWS_SECRET_ACCESS_KEY

RUN export AWS_ACCESS_KEY_ID=$(cat /etc/secrets/AWS_ACCESS_KEY_ID) && \
    export AWS_SECRET_ACCESS_KEY=$(cat /etc/secrets/AWS_SECRET_ACCESS_KEY)
